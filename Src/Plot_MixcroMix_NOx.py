...
Zt      = 0.02
iz1D    = np.argmax((Zs1D > Zt)) ; print("In 1D: Z = ", Zs1D[iz1D])
...
labelsize = 20; lw = 3.0
ax.plot(pv1D, Y_1D[:, idx_NO], linestyle="--", color="r", linewidth = lw)
ax.plot(pv_C, Y_C[:, idx_NO], linestyle="-", color="k", linewidth = lw)
ax.set_ylim([0, 1E-4])
ax.set_xlabel(r"$C$", fontsize = 20)
ax.set_ylabel(r"$Y_\mathrm{NO}$", fontsize = 20)
...
fig, ax = plt.subplots(figsize=(4,3))
labelsize = 20; lw = 3.0
ax.plot(agepv_1D*1E3, fstate.Y[indx_pf,idx_NO], linestyle="--", color="r", linewidth = lw)
ax.plot(agepv_A*1E3, Y_A[:, idx_NO], linestyle="-", color="k", linewidth = lw)
ax.set_xlim([0, 1.5E-1])
ax.set_ylim([0, 1.5E-4])
ax.set_xlabel(r"$A_\mathrm{p}~\mathrm{[ms]}$", fontsize = 20)
ax.set_ylabel(r"$Y_\mathrm{NO}$", fontsize = 20)
ax.ticklabel_format(axis='y', style='sci', scilimits=(3,1))
ax.tick_params(axis='both', which='major', labelsize=labelsize)
ax.tick_params(axis='both', which='minor', labelsize=labelsize)
...
fig, ax = plt.subplots(figsize=(4,3))
labelsize = 20; lw = 3.0
ax.plot(agepv_1D*1E3, fstate.T[indx_pf], linestyle="--", color="r", linewidth = lw)
ax.plot(agepv_A*1E3, T_A[:], linestyle="-", color="k", linewidth = lw)
ax.set_xlim([0, 1.5E-1])
#ax.set_ylim([0, 1.5E-4])
ax.set_xlabel(r"$A_\mathrm{p}~\mathrm{[ms]}$", fontsize = 20)
...
fig, ax = plt.subplots(figsize=(4,3))
labelsize = 20; lw = 3.0
NO_ID = gas1D.species_index("NO")
wdot_favm = wdot_A[:, NO_ID] / rho_A
ax.plot(agepv_A*1E3, wdot_favm, linestyle="-", color="k", linewidth = lw)
ax.plot(agepv_1D*1E3, net_production_rates[indx_pf,NO_ID]*gas1D.molecular_weights[NO_ID],
        linestyle="--", color="r", linewidth = lw)
ax.set_xlim([0, 1.5E-1])
#ax.set_ylim([0, 1.5E-4])
ax.set_xlabel(r"$A_\mathrm{p}~\mathrm{[ms]}$", fontsize = 20)
ax.set_ylabel(r"$\dot{\omega}_\mathrm{NO}$", fontsize = 20)
ax.ticklabel_format(axis='y', style='sci', scilimits=(3,1))
ax.tick_params(axis='both', which='major', labelsize=labelsize)
ax.tick_params(axis='both', which='minor', labelsize=labelsize)
...
# <Y(NO)|A> over all Z
axis_ZC = (0,1)
wt_A_wtsum    = np.sum(wt_ZCA_wtsum[:,:,:], axis = axis_ZC)
rho_A_wtsum   = np.sum(rho_ZCA_wtsum[:,:,:], axis = axis_ZC)
pv_A_wtsum    = np.sum(pv_ZCA_wtsum[:,:,:], axis = axis_ZC)
rhoT_A_wtsum  = np.sum(rhoT_ZCA_wtsum[:,:,:], axis = axis_ZC)
agepv_A_wtsum = np.sum(agepv_ZCA_wtsum[:,:,:], axis = axis_ZC)
sl = (wt_A_wtsum > 0)
rho_A      = rho_A_wtsum[sl] / wt_A_wtsum[sl]
rhoT_A     = rhoT_A_wtsum[sl] / wt_A_wtsum[sl]
agepv_A    = agepv_A_wtsum[sl] / wt_A_wtsum[sl]
pv_A       = pv_A_wtsum[sl] / wt_A_wtsum[sl]
rhoY_A     = np.zeros((rho_A.shape[0], Nsp))
wt_A      = np.zeros((rho_A.shape[0], Nsp))
Y_A        = np.zeros_like(rhoY_A)
for isp in range(0, Nsp):
  rhoY_A[:, isp] = np.sum(rhoY_ZCA_wtsum[iz3D,:,:,isp], axis = axis_C)[sl] / wt_A_wtsum[sl]
  wt_A[:, isp] = np.sum(wdot_ZCA_wtsum[iz3D,:,:,isp], axis = axis_C)[sl] / wt_A_wtsum[sl]
  Y_A[:, isp] = rhoY_A[:, isp] / rho_A[:]
T_A = rhoT_A / rho_A
