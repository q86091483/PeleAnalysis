...
rhoY_H2_wtsum = np.zeros((nx, ny, nz, nmf, npv))
rhoY_O2_wtsum = np.zeros((nx, ny, nz, nmf, npv))
rhoY_H2O_wtsum = np.zeros((nx, ny, nz, nmf, npv))
rhoY_OH_wtsum = np.zeros((nx, ny, nz, nmf, npv))
rhoY_NO_wtsum = np.zeros((nx, ny, nz, nmf, npv))
rhoY_N2O_wtsum = np.zeros((nx, ny, nz, nmf, npv))
rhoY_NNH_wtsum = np.zeros((nx, ny, nz, nmf, npv))
...
rhoY_H2_xyzZC_wtsum = rhoY_H2_wtsum + f["DATA"]["rhoY(H2)_mean"]
rhoY_O2_xyzZC_wtsum = rhoY_O2_wtsum + f["DATA"]["rhoY(O2)_mean"]
rhoY_H2O_xyzZC_wtsum = rhoY_H2O_wtsum + f["DATA"]["rhoY(H2O)_mean"]
rhoY_OH_xyzZC_wtsum = rhoY_OH_wtsum + f["DATA"]["rhoY(OH)_mean"]
rhoY_NO_xyzZC_wtsum = rhoY_NO_wtsum + f["DATA"]["rhoY(NO)_mean"]
rhoY_N2O_xyzZC_wtsum = rhoY_N2O_wtsum + f["DATA"]["rhoY(N2O)_mean"]
rhoY_NNH_xyzZC_wtsum = rhoY_NNH_wtsum + f["DATA"]["rhoY(NNH)_mean"]
...
rhoY_H2_ZC_sum = np.sum(rhoY_H2_xyzZC_wtsum, axis=axis_sum)
rhoY_O2_ZC_sum = np.sum(rhoY_O2_xyzZC_wtsum, axis=axis_sum)
rhoY_OH_ZC_sum = np.sum(rhoY_OH_xyzZC_wtsum, axis=axis_sum)
rhoY_H2O_ZC_sum = np.sum(rhoY_H2O_xyzZC_wtsum, axis=axis_sum)
rhoY_NO_ZC_sum = np.sum(rhoY_NO_xyzZC_wtsum, axis=axis_sum)
rhoY_N2O_ZC_sum = np.sum(rhoY_N2O_xyzZC_wtsum, axis=axis_sum)
rhoY_NNH_ZC_sum = np.sum(rhoY_NNH_xyzZC_wtsum, axis=axis_sum)
...
rhoY_H2_ZC = rhoY_H2_ZC_sum / wt_ZC_sum
rhoY_O2_ZC = rhoY_O2_ZC_sum / wt_ZC_sum
rhoY_OH_ZC = rhoY_OH_ZC_sum / wt_ZC_sum
rhoY_H2O_ZC = rhoY_H2O_ZC_sum / wt_ZC_sum
rhoY_NO_ZC = rhoY_NO_ZC_sum / wt_ZC_sum
rhoY_N2O_ZC = rhoY_N2O_ZC_sum / wt_ZC_sum
rhoY_NNH_ZC = rhoY_NNH_ZC_sum / wt_ZC_sum
...
ax.set_xlim([0.0, 1.0])
ax.set_ylabel(r"$C$", fontsize=labelsize)
ax.set_title(r"$\rho Y_\mathrm{OH}\; | \; Z, C \rangle~\mathrm{[K]}$", fontsize=labelsize)
ax.tick_params(axis='both', which='major', labelsize=labelsize-2)
ax.tick_params(axis='both', which='minor', labelsize=labelsize-2)

#%%
fig, ax = plt.subplots(figsize=figsize)
phi = np.abs(rhoY_H2O_ZC)
ax.imshow(phi.transpose(), origin="lower", 
          # vmin=0, vmax=5.0, 
          cmap = "jet",
          extent = [0.0, 1.0, 0.0, 1.2])
...
ax.set_xlim([0.0, 1.0])
ax.set_ylabel(r"$C$", fontsize=labelsize)
ax.set_title(r"$\langle \rho Y_\mathrm{H2O}\; | \; Z, C \rangle~\mathrm{[K]}$", fontsize=labelsize)
ax.tick_params(axis='both', which='major', labelsize=labelsize-2)
ax.tick_params(axis='both', which='minor', labelsize=labelsize-2)

#%%
fig, ax = plt.subplots(figsize=figsize)
phi = np.abs(rhoY_H2_ZC)
ax.imshow(phi.transpose(), origin="lower", 
          # vmin=0, vmax=5.0, 
          cmap = "jet",
          extent = [0.0, 1.0, 0.0, 1.2])
...
ax.set_xlim([0.0, 1.0])
ax.set_ylabel(r"$C$", fontsize=labelsize)
ax.set_title(r"$\langle \rho Y_\mathrm{H2}\; | \; Z, C \rangle~\mathrm{[K]}$", fontsize=labelsize)
ax.tick_params(axis='both', which='major', labelsize=labelsize-2)
ax.tick_params(axis='both', which='minor', labelsize=labelsize-2)
#%%
fig, ax = plt.subplots(figsize=figsize)
phi = np.abs(rhoY_NNH_ZC)
ax.imshow(phi.transpose(), origin="lower", 
          # vmin=0, vmax=5.0, 
          cmap = "jet",
          extent = [0.0, 1.0, 0.0, 1.2])
...
ax.set_xlim([0.0, 1.0])
ax.set_ylabel(r"$C$", fontsize=labelsize)
ax.set_title(r"$\langle \rho Y_\mathrm{NNH}\; | \; Z, C \rangle~\mathrm{[K]}$", fontsize=labelsize)
ax.tick_params(axis='both', which='major', labelsize=labelsize-2)
ax.tick_params(axis='both', which='minor', labelsize=labelsize-2)

#%%
fig, ax = plt.subplots(figsize=figsize)
phi = np.abs(rhoY_NO_ZC)
ax.imshow(phi.transpose(), origin="lower", 
          vmin=0, vmax=1.0E-5, 
          cmap = "jet",
          extent = [0.0, 1.0, 0.0, 1.2])
...
ax.set_xlim([0.0, 1.0])
ax.set_ylabel(r"$C$", fontsize=labelsize)
ax.set_title(r"$\langle \rho Y_\mathrm{NO}\; | \; Z, C \rangle~\mathrm{[K]}$", fontsize=labelsize)
ax.tick_params(axis='both', which='major', labelsize=labelsize-2)
ax.tick_params(axis='both', which='minor', labelsize=labelsize-2)
