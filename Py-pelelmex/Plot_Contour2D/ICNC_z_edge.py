  ...
  for ipy in range(0, npy):
    for ipx in range(0, npx):
      ...
      if (field_name == "HeatRelease"):
        ...
        ax.text(-10, y=5.5, s=r"$t="+("%.3f"%(time*1000))+"\mathrm{[ms]}$", c="black", fontsize=labelsize-4)
        if plane_info != None:
          ax.text(x=-10, y=7.3, s=plane_info, fontsize = labelsize-4)
        if time >= tfine:
          finest_level = 3
        else: 
          finest_level = 2
        if True:
          ax.text(x=-10, y=9.1, s=r"$\mathrm{Finest \; level}="+str(finest_level)+"$", fontsize = labelsize-4)
