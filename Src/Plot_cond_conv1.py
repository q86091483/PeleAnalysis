...
for ipx in range(0, nrows):
    for ipy in range(0, ncols):
      ax = axs[ipx, ipy]
      idx = ipx*2 + ipy
      phi = f["DATA"]["uY("+spn+")_sum"][:,idx,idy,idz]
      ax.plot(zbin, phi, linewidth=linewidth, color='b', label="convev")

      phi_sdr = f["DATA"]["rhoD("+spn+")_mean"][:,idx,idy,idz];
      phi_pdf = f["DATA"]["volume_sum"][:,idx,idy,idz]
      ax.plot(zbin, phi_sdr, linewidth=linewidth, color='k', label="SDR") 

      Geta=np.exp(-2*(special.erfinv(2*zbin/zbin[-1]-1))**2)
      intG = np.sum(Geta*0.13/100)
      sdr_avg = np.sum(phi_sdr * phi_pdf / np.sum(phi_pdf))
      sdr_fit = Geta * sdr_avg / intG
      ax.plot(zbin, sdr_fit, linewidth=linewidth, linestyle='--', color='k', label="SDR") 
      ...
      if (ipy == 0):
         ...
         s = "s"
         ax.set_ylabel(r"$\langle N_{H2O} | Z \rangle_\delta$", fontsize=labelsize)
...
plt.savefig("SDR_Z.png", dpi=300, bbox_inches="tight")
#ax2 = ax.twinx()
