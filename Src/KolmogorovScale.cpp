...
int main (int argc, char* argv[])
{
  ...
  for (int iPlot; iPlot < nPlotFiles; ++iPlot) {
    ...
    if (isioproc) {
      ...
      writeIntData(h5domain, "count_lvl", Vector<int>(1, nLevels), countLvl,
                   0.0);
      for (auto &v : volLvl)
        v *= vol0;
      writeRealData(h5domain, "volume_lvl", Vector<int>(1, nLevels), volLvl,
                    0.0);
      h5domain.close();
