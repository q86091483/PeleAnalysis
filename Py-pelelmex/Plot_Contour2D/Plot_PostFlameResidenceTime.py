...
data_pattern = "plt_17200*"
# Field_names
field_names = [["agepv_1_bin", "Y(NNH)", "Y(NO)"],]

# Output data folder
fig_dir = os.path.join(path_PeleAnalysis, "Figure")
fig_slice_dir = os.path.join(fig_dir, "Slice2D_age_lev1")
...
for ifn, fn in enumerate(fns_sorted[0:]):
  ...
  for ipy in range(0, npy):
    for ipx in range(0, npx):
      if (npy == 1 and npx == 1):
        ax = axs
      elif (npy == 1 and npx > 1):
        ax = axs[ipx]
      elif (npx == 1 and npy > 1):
        ax = axs[ipy]
      else:
        ax = axs[ipy, ipx]
      ...
      if (field_name == "agepv_1_bin"):
        vmin = 0; vmax = 1.2E-4
        rhoMixf = f["mixture_fraction_userdef_1"] + np.amax(f["mixture_fraction_userdef_1"]) * eps_age
        agepv_1 = f["agepv_1"] / rhoMixf
        agepv_1_bin = np.zeros_like(agepv_1)
        bin_agepv = np.linspace(0, 1.2E-4, 12)
        for ib, b in enumerate(bin_agepv[0:-1]):
          idx = (agepv_1 < bin_agepv[ib+1]) & (agepv_1 >= bin_agepv[ib])
          agepv_1_bin[idx] = ib
        ...
        colors = []
        for i in indx:
          colors.append(tab20c(i))
        cmap=matplotlib.colors.ListedColormap(colors)

        im = ax.imshow(agepv_1, origin="lower",
