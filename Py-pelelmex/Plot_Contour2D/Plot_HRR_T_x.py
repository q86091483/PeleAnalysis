...
data_folder = ("/scratch/b/bsavard/zisen347/PeleAnalysis/"
              "Py-pelelmex/Data/Slice2D/Micromix/HRR_T_x=4.500E-03")
# Case name
case_name = "Micromix"
str_plane = "HRR_T_x=4.500E-03"
str_info = r"$x/D_{jet}=10$"
# Patterns of plotfiles to be processed
data_pattern = "plt_*"
# Field_names
field_names = [["HeatRelease", "Y(NO)", "temp"], 
               ["temp", "Y(N)", "y_velocity"],
...
#%%
for ifn, fn in enumerate(fns_sorted[-1:]):
  ...
  f = np.load(fn)
  y1D = f["x"]; Ly = y1D[-1] - y1D[0]
  z1D = f["y"]; Lz = z1D[-1] - z1D[0]
  extent=np.array([-y1D[-1], y1D[-1], 0, z1D[-1]]) / Djet

  figsize = ((Ly/Lz)*fig_unit_y*npx*1.05, fig_unit_y*npy)
  ...
  for ipy in range(0, npy):
    for ipx in range(0, npx):
      ...
      if (field_name == "HeatRelease"):
        ...
        ax.text(x=12.0, y=15.0, s=r"$t="+("%.3f"%(time*1000))+"\mathrm{[ms]}$", c="black")
        if str_info != None:
      ...
      if (ipy == npy-1):
        ax.set_xlabel(r"$y/D_{jet}$", fontsize = labelsize)
        ax.set_xticks(np.array([-4.0, 4.0]))
      else:
        #ax.set_xlabel([])
