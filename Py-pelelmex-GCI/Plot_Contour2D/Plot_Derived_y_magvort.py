...
data_pattern = "plt_*"
# Field_names
field_names = [["mag_vort"],]
# Output data folder
fig_dir = os.path.abspath("../Figure")
fig_slice_dir = os.path.join(fig_dir, "Slice2D_magvort_lev2")
  ...
  for ipy in range(0, npy):
    for ipx in range(0, npx):
      ...
      if (field_name == "mag_vort"):
        ...
        ax.set_title(r"$|\omega| \; \mathrm{[1/s]}$", fontsize=labelsize-2, pad=3)

        cax = ax.inset_axes(loc_cb, transform=ax.transData)
        cb = fig.colorbar(im, cax=cax, orientation='horizontal',
                          ticks=[vmin, vmax])
        cb.ax.xaxis.set_tick_params(color="magenta")
        cb.ax.yaxis.set_tick_params(color="magenta")
        cb.outline.set_edgecolor("magenta")
        cb.ax.set_xticklabels([r"$10^3$", r"$10^6$"], color="magenta", fontsize=labelsize-4)  # horizontal colorbar
